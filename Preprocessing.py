...
@contextmanager
def timer(title):
    # Affiche le temps écoulé lors de l'exécution des fonctions
    t0 = time.time()
    yield
...
def one_hot_encoder(df, nan_as_category = True):
    # Remplace les colonnes catégorielles à l'aide d'un one-hot encoder
...
def application_train(num_rows=None, nan_as_category=False):
    ...
    df['DAYS_EMPLOYED_PERC'] = df['DAYS_EMPLOYED'] / df['DAYS_BIRTH'] # Pourcentage de temps de vie passé dans l'entreprise actuelle
    df['INCOME_CREDIT_PERC'] = df['AMT_INCOME_TOTAL'] / df['AMT_CREDIT'] # Revenus / Montant du crédit
...
def kfold_lightgbm(df, num_folds, stratified=False, debug=False):
    ...
    if not debug:
        ...
        df[liste_best_features40].to_csv('df_best40.csv', index=False)
        ...
        df[liste_best_features80].to_csv('df_best80.csv', index=False)
...
def display_importances(feature_importance_df_):
    ...
    sns.barplot(x="importance", y="feature", data=best_features40.sort_values(by="importance", ascending=False))
